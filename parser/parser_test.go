...
func TestStatementBoundaries(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			"let x: int = 5;",
			token.Position{Line: 0, Column: 0},
			token.Position{Line: 0, Column: 15},
		},
		{
			"Let Statement with spaces and types",
			`   let  y : string  =  "hello" ;   `,
			token.Position{Line: 0, Column: 3},
		...
		},
		{
			...
			"return 100;",
			token.Position{Line: 0, Column: 0},
			token.Position{Line: 0, Column: 11},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(program.Statements) == 0 {
				t.Fatalf("no statements parsed for input: %s", tt.input)
			}

			stmt := program.Statements[0]

			if stmt.Start() != tt.expectedStart {
				t.Errorf("Statement Start wrong. expected=%+v, got=%+v",
					tt.expectedStart, stmt.Start())
			}

			if stmt.End() != tt.expectedEnd {
				t.Errorf("Statement End wrong. expected=%+v, got=%+v",
					tt.expectedEnd, stmt.End())
			}
		})
	}
}
